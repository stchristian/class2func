    ...
    return (
      <div className="ContactPage">
        ...
        <div className="service-places">
          {contact.items.map((item, index) => this.renderServicePlace(item.name, item.phoneNumber, item.email, index))}
        </div>
        <div className="page-title">
          <h1>{faq.title}</h1>
        </div>
        <div className="faq-list">
          {faq.items &&
            faq.items.map((category, catIndex) => (
              <div key={catIndex}>
                <h2>{category.title}</h2>
                {category.items &&
                  category.items.map((item, itemIndex) => (
                    <Dropdown key={category.name + itemIndex} title={item.title}>
